...
dojo.provide("RichText.widget.MobileRichTextViewer");

//default editor CSS
mendix.dom.insertCss(mx.moduleUrl('RichText') + 'widget/styles/contents.css');

dojo.declare('RichText.widget.MobileRichTextViewer', {
	addons: [ "mxui.mixin._Scriptable" ],
	height    : 120,
	...
	createMicroflowLinks : function(text) {
		...
		for(var i = 0; i < this.functionNames.length; i++) 
			if (dojo.isString(this.functionNames[i]))	
				text = text.replace(new RegExp("\\$\\{"+this.functionNames[i]+"\\}","gi"), "typeof(dijit) != &quot;undefined&quot; && dijit.byId(&quot;"+this.id+"&quot;).invokecallback("+i+")");
		return text;
	},
	...
	invokecallback : function(idx) {
		...
		if (idx >= this.functionMicroflows.length)
			...
			logger.error(this.id + "No context");
			throw this.id + " No object in context, aborting.";
		}
	},
